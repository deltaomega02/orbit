...
> **발표자료와 코드의 모델명이 다릅니다.** 결과보고서에는 `Gemini 3.0`으로 적혀 있는데,
> 이는 프로젝트 막바지에 자료를 만들면서 당시 검토 중이던 모델명을 넣은 것입니다.
> **실제 코드가 호출하는 모델은 `gemini-2.5-pro`와 `gemini-2.5-flash-image-preview`이고**,
> 이 README를 포함한 문서는 코드와 대조해 실제 사용 모델로 정정했습니다.
> 발표자료는 제출 원본이라 그대로 두었습니다 — 자료보다 코드가 사실입니다.
